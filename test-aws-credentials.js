...
console.log('Has AWS_CONTAINER_CREDENTIALS_RELATIVE_URI:', !!process.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI)
console.log()
...
if (!process.env.AWS_ACCESS_KEY_ID && !process.env.AWS_PROFILE) {
  ...
  console.log('   3. EC2 instance metadata (if running on EC2) <- CDP uses this')
  console.log()
}
...
const inferenceProfileArn = 'arn:aws:bedrock:eu-west-2:332499610595:inference-profile/eu.anthropic.claude-3-5-sonnet-20241022-v2:0'
const guardrailArn = 'arn:aws:bedrock:eu-west-2:332499610595:guardrail/j7sbivk41lq4'
...
try {
  ...
  const command = new ConverseCommand({
    ...
    messages: [
      {
        role: 'user',
        content: [{ text: 'Hello, respond with just "OK" if you can read this.' }]
      }
    ],
    inferenceConfig: {
      maxTokens: 10,
  ...
  console.log('Response:', response.output?.message?.content?.[0]?.text || '[no text]')
  ...
  if (error.name === 'CredentialsProviderError') {
    ...
    console.error('Solution: In CDP, ensure the EC2 instance has an IAM role with Bedrock permissions')
  } else if (error.name === 'AccessDeniedException') {
    console.error('Root Cause: Credentials found but lack permissions')
    console.error('Solution: Add bedrock:InvokeModel permission to the IAM role/user')
  } else if (error.name === 'ResourceNotFoundException') {
    console.error('Root Cause: Inference profile or guardrail not found')
    console.error('Solution: Verify the ARNs are correct for this region/account')
...
console.log('  - Credential errors are EXPECTED unless you have AWS credentials configured')
console.log('  - To configure: aws configure (using AWS CLI) or set environment variables')
console.log()
...
console.log('    4. Security groups allow access to metadata endpoint (169.254.169.254)')

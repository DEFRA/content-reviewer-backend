...
function buildFollowUpPrompt(canonicalText, thirdIndex, docLength) {
  ...
  return [
    ...
    '- CURRENT and SUGGESTED must be different — omit any issue where they would be identical',
    '- Only flag real issues; do not manufacture problems',
    '- Do NOT output a [SCORES] section — scores are already decided',
    '- Output ONLY [IMPROVEMENTS] in the standard format',
    '',
    '<content_to_review>',
    canonicalText,
    '</content_to_review>',
...
export class BedrockReviewProcessor {
  ...
  buildUserPrompt(textContent) {
    ...
    return [
      ...
      '',
      `DOCUMENT THIRDS (character offsets, min ${minIssuesPerThird} issue each):`,
      `  first:  0 – ${firstThirdEnd - 1}`,
      `  middle: ${middleThirdStart} – ${middleThirdEnd - 1}`,
      `  final:  ${finalThirdStart} – ${documentLength - 1}`,
      '',
      'REMINDERS:',

...
describe('Upload Routes - uploadRoutes plugin', () => {
  ...
  describe('POST /upload-callback - handleUploadCallback', () => {
    ...
    it('logs processing details at info level', async () => {
      reviewHelpers.createCanonicalDocument.mockResolvedValueOnce({
        canonicalResult: {
          s3: { bucket: 'test-bucket', key: 'documents/doc-123.json' },
          document: { charCount: 1000 }
        },
        canonicalDuration: 80
      })
      reviewHelpers.createReviewRecord.mockResolvedValueOnce(20)
      reviewHelpers.queueReviewJob.mockResolvedValueOnce(10)
      ...
      await server.inject({
        ...
        payload: {
          ...
          numberOfRejectedFiles: 0,
          metadata: {
            reviewId: 'review-processing-log',
      ...
      expect(logger.info).toHaveBeenCalledWith(
        ...
        expect.stringContaining('Processing uploaded file')
      )
    })

    it('logs error when uploadStatus validation fails', async () => {
      await server.inject({
        method: 'POST',
        url: '/upload-callback',
        payload: {
    ...
    it('logs error when files are rejected', async () => {
      await server.inject({
        method: 'POST',
        url: '/upload-callback',
        payload: {
